        ...
        tasks.map((task) => (
          <div key={task.id} className="flex flex-col gap-2 mt-2 p-3 text-white bg-green-700 rounded-md shadow-md">
            ...
            <div className="flex w-full justify-end items-center gap-4 mt-4">
              <button className="btn btn-primary text-white flex gap-1 px-3" onClick={() => handledEdit(task)}>
                <FaRegEdit className="text-base" />
                Edit
              </button>

              <button className="btn btn-error bg-red-600 text-white flex gap-1 px-3" onClick={() => handleDelete(task.id)}>
